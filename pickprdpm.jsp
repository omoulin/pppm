    ...
    <%
      ...
      if (Userlogin==null) {
        ...
        if (ptmp==null) {
      	...
      	out.print("<center><strong><h1>Product Manager</h1></strong></center><hr>");
        out.print("You are here : Portfolio \\ <A HREF=\"productportfolio.jsp?TODO=NONE\" target=\"appliFrame\"> Products</A> \\ <A HREF=\"productdetails.jsp?TODO=NONE&PRODUCTID="+PRODUCTID+"\" target=\"appliFrame\">Product details</A> \\ Add PM");
        out.print("<hr>");
        out.print("<form name=\"formFILTER\" method=\"post\" action=\"pickprdpm.jsp?TODO=FILTER&FILTER=FILTER&PRODUCTID="+PRODUCTID+"&PAGE=0\" target=\"appliFrame\">");
        String LASTFILTERNAME=null;
        Statement st123b = Conn.createStatement();
        String q123b = "SELECT FILTERVALUE FROM FILTER WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERNAMEPICKPRODUCTPM'";
        ...
        String q123c = "SELECT FILTERVALUE FROM FILTER WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERFORNAMEPICKPRODUCTPM'";
        ...
        if (filstrNAME==null) {
          ...
          if (LASTFILTERNAME!=null) {
            ...
            String i2 = "UPDATE FILTER SET FILTERVALUE='"+filstrNAME+"' WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERNAMEPICKPRODUCTPM'";
            sti2.executeUpdate(i2);
            sti2.close();
          } else {
            Statement sti2 = Conn.createStatement();
            String i2 = "INSERT INTO FILTER VALUES('"+Userlogin+"','LASTFILTERNAMEPICKPRODUCTPM','"+filstrNAME+"')";
            sti2.executeUpdate(i2);
            sti2.close();
          }
        }
        if (filstrFORNAME==null) {
          ...
          if (LASTFILTERFORNAME!=null) {
            Statement sti2 = Conn.createStatement();
            String i2 = "UPDATE FILTER SET FILTERVALUE='"+filstrFORNAME+"' WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERFORNAMEPICKPRODUCTPM'";
            sti2.executeUpdate(i2);
            sti2.close();
          } else {
            Statement sti2 = Conn.createStatement();
            String i2 = "INSERT INTO FILTER VALUES('"+Userlogin+"','LASTFILTERFORNAMEPICKPRODUCTPM','"+filstrFORNAME+"')";
            sti2.executeUpdate(i2);
            sti2.close();
          }
        }
        ...
        String q230 = "SELECT COUNT(ID) FROM EMPLOYEE WHERE "+filterstatement+" ID NOT IN (SELECT EMPLOYEEID FROM PRDPM WHERE PRODUCTID='"+PRODUCTID+"')";
        ...
        for (int i=0; i<=nbpage;i++) {
          out.print("<td><A HREF=\"pickproductpm.jsp?TODO=ADD&PAGE="+i+"&PRODUCTID="+PRODUCTID+"\">"+i+"</A></td>");
        }
        out.print("</tr></table>");
        out.print("<table border=\"0\" CELLSPACING=\"2\" BORDERCOLOR=\"#000000\" width=\"100%\">");
        out.print("<tr height=\"30\">");
        out.print("<td width=\"10%\">&nbsp;</td>");
        out.print("<td width=\"80%\" background=\"images/fond_titre.jpg\"><center>Project Manager</center></td>");
        out.print("<td width=\"10%\">&nbsp;</td>");
        out.print("</tr>");
        Statement STR01 = Conn.createStatement();
        String QR01 = "SELECT ID,NAME,FORNAME FROM EMPLOYEE WHERE "+filterstatement+" ID NOT IN (SELECT EMPLOYEEID FROM PRDPM WHERE PRODUCTID='"+PRODUCTID+"')";
        ...
        while(R01.next()) {
          if (pagecount>=PAGE*20 && pagecount<(PAGE+1)*20) {
            out.print("<tr>");
            out.print("<td>&nbsp;</td>");
            out.print("<td bgcolor=\"#FFFFFF\"><A HREF=\"productdetails.jsp?TODO=PMADD&ID="+R01.getString("ID")+"&PRODUCTID="+PRODUCTID+"\">"+R01.getString("NAME")+" "+R01.getString("FORNAME")+"</td>");
            out.print("<td>&nbsp;</td>");
            out.print("</tr>");
          }
          pagecount++;

    ...
    <%
      ...
      if (Userlogin==null) {
        ...
        if (ptmp==null) {
      	...
      	out.print("<center><strong><h1>Application Owner</h1></strong></center><hr>");
        out.print("<form name=\"formFILTER\" method=\"post\" action=\"pickapplicationpm.jsp?TODO=FILTER&FILTER=FILTER&APPLICATIONID="+APPLICATIONID+"&PAGE=0\" target=\"_top\">");
        String LASTFILTERNAME=null;
        Statement st123b = Conn.createStatement();
        String q123b = "SELECT FILTERVALUE FROM FILTER WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERNAMEPICKAPPLICATIONPM'";
        ...
        String q123c = "SELECT FILTERVALUE FROM FILTER WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERFORNAMEPICKAPPLICATIONPM'";
        ...
        if (filstrNAME==null) {
          ...
          if (LASTFILTERNAME!=null) {
            ...
            String i2 = "UPDATE FILTER SET FILTERVALUE='"+filstrNAME+"' WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERNAMEPICKAPPLICATIONPM'";
            sti2.executeUpdate(i2);
            sti2.close();
          } else {
            Statement sti2 = Conn.createStatement();
            String i2 = "INSERT INTO FILTER VALUES('"+Userlogin+"','LASTFILTERNAMEPICKAPPLICATIONPM','"+filstrNAME+"')";
            sti2.executeUpdate(i2);
            sti2.close();
          }
        }
        if (filstrFORNAME==null) {
          ...
          if (LASTFILTERFORNAME!=null) {
            Statement sti2 = Conn.createStatement();
            String i2 = "UPDATE FILTER SET FILTERVALUE='"+filstrFORNAME+"' WHERE LOGIN='"+Userlogin+"' AND FILTERNAME='LASTFILTERFORNAMEPICKAPPLICATIONPM'";
            sti2.executeUpdate(i2);
            sti2.close();
          } else {
            Statement sti2 = Conn.createStatement();
            String i2 = "INSERT INTO FILTER VALUES('"+Userlogin+"','LASTFILTERFORNAMEPICKAPPLICATIONPM','"+filstrFORNAME+"')";
            sti2.executeUpdate(i2);
            sti2.close();
          }
        }
        ...
        String q230 = "SELECT COUNT(ID) FROM EMPLOYEE WHERE "+filterstatement+" ID NOT IN (SELECT EMPLOYEEID FROM APPPM WHERE APPLICATIONID='"+APPLICATIONID+"')";
        ...
        for (int i=0; i<=nbpage;i++) {
          out.print("<td><A HREF=\"pickapplicationpm.jsp?TODO=ADD&PAGE="+i+"&APPLICATIONID="+APPLICATIONID+"\">"+i+"</A></td>");
        }
        out.print("</tr></table>");
        out.print("<table border=\"0\" CELLSPACING=\"2\" BORDERCOLOR=\"#000000\" width=\"100%\">");
        out.print("<tr height=\"30\">");
        out.print("<td width=\"10%\">&nbsp;</td>");
        out.print("<td width=\"80%\" background=\"images/fond_titre.jpg\"><center>Application Owner</center></td>");
        out.print("<td width=\"10%\">&nbsp;</td>");
        out.print("</tr>");
        Statement STR01 = Conn.createStatement();
        String QR01 = "SELECT ID,NAME,FORNAME FROM EMPLOYEE WHERE "+filterstatement+" ID NOT IN (SELECT EMPLOYEEID FROM APPPM WHERE APPLICATIONID='"+APPLICATIONID+"')";
        ...
        while(R01.next()) {
          if (pagecount>=PAGE*20 && pagecount<(PAGE+1)*20) {
            out.print("<tr>");
            out.print("<td>&nbsp;</td>");
            out.print("<td bgcolor=\"#FFFFFF\"><A HREF=\"applicationdetails.jsp?TODO=PMADD&ID="+R01.getString("ID")+"&APPLICATIONID="+APPLICATIONID+"\">"+R01.getString("NAME")+" "+R01.getString("FORNAME")+"</td>");
            out.print("<td>&nbsp;</td>");
            out.print("</tr>");
          }
          pagecount++;
        ...
        STR01.close();
        out.print("</table>");
        out.print("<A HREF=\"applicationdetails.jsp?TODO=NONE&APPLICATIONID="+APPLICATIONID+"\"><img border=0 src=\"icons/cancel.png\"></A>");
        Conn.close();
      }
